...
func (s *UploadRecordService) GetStatistics(startDate, endDate, projectName, dataType, status, uploader string) (*dto.UploadRecordStatisticsResp, error) {
	...
	byStatus, _ := s.uploadRecordRepo.CountByStatus(filterNoDate)
	resp.ByStatus = byStatus

	byDataType, _ := s.uploadRecordRepo.CountByDataType(filterNoDate)
	resp.ByDataType = byDataType

	byProject, _ := s.uploadRecordRepo.CountByProject(repo.StatisticsFilter{
		DataType: dataType,
	})
